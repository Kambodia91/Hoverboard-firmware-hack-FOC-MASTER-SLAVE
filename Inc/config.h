...
#define DEAD_TIME              48     // PWM deadtime 48
...
#define BAT_CALIB_REAL_VOLTAGE  3947      // input voltage measured by multimeter (multiplied by 100). In this case 43.00 V * 100 = 4300
#define BAT_CALIB_ADC           1495      // adc-value measured by mainboard (value nr 5 on UART debug output)
...
// ############################### DEBUG SERIAL ###############################
/* Connect GND and RX of a 3.3v uart-usb adapter to the left (USART2) or right sensor board cable (USART1)
 ...
 * enable DEBUG_SERIAL_USART1 or DEBUG_SERIAL_USART2
 *
 *
...
// #define DEBUG_SERIAL_USART1          // right sensor board cable, disable if I2C (nunchuk or lcd) is used!
...
*/
  // #define DUAL_INPUTS                     // ADC*(Primary) + PPM(Auxiliary). Uncomment this to use Dual-inputs
    ...
    #define CONTROL_ADC           0       // use ADC as input. Number indicates priority for dual-input. Disable CONTROL_SERIAL_USART2, FEEDBACK_SERIAL_USART2, DEBUG_SERIAL_USART2!
    #define CONTROL_PPM_RIGHT     1       // use PPM-Sum as input on the RIGHT cable. Number indicates priority for dual-input. Disable CONTROL_SERIAL_USART1!
    #define PRI_INPUT1            3,     0, 0, 4095,   0  // TYPE, MIN, MID, MAX, DEADBAND. See INPUT FORMAT section
    #define PRI_INPUT2            3,     0, 0, 4095,   0  // TYPE, MIN, MID, MAX, DEADBAND. See INPUT FORMAT section
    ...
    #define CONTROL_PPM_RIGHT     0       // use PPM-Sum as input on the RIGHT cable. Number indicates priority for dual-input. Disable CONTROL_SERIAL_USART1!
...
#ifdef VARIANT_PWM
/* ###### CONTROL VIA RC REMOTE ######
 * Right sensor board cable. Connect PB6 to channel 1 and PB7 to channel 2 on receiver.
 * Channel 1: MasterSpeed, Channel 2: SlaveSpeed.
*/
  // #define DUAL_INPUTS                     // ADC*(Primary) + PWM(Auxiliary). Uncomment this to use Dual-inputs
    ...
    #define CONTROL_ADC           0       // use ADC as input. Number indicates priority for dual-input. Disable CONTROL_SERIAL_USART2, FEEDBACK_SERIAL_USART2, DEBUG_SERIAL_USART2!
    #define CONTROL_PWM_RIGHT     1       // use RC PWM as input on the RIGHT cable. Number indicates priority for dual-input. Disable DEBUG_SERIAL_USART1!
    #define PRI_INPUT1            3,     0, 0, 4095,   0  // TYPE, MIN, MID, MAX, DEADBAND. See INPUT FORMAT section
    #define PRI_INPUT2            3,     0, 0, 4095,   0  // TYPE, MIN, MID, MAX, DEADBAND. See INPUT FORMAT section
    #define AUX_INPUT1            3, -1000, 0, 1000, 100  // TYPE, MIN, MID, MAX, DEADBAND. See INPUT FORMAT section
    #define AUX_INPUT2            3, -1000, 0, 1000, 100  // TYPE, MIN, MID, MAX, DEADBAND. See INPUT FORMAT section
    ...
    #define CONTROL_PWM_RIGHT     0       // use RC PWM as input on the RIGHT cable. Number indicates priority for dual-input. Disable DEBUG_SERIAL_USART1!
    #define PRI_INPUT1            3, -1000, 0, 1000, 100  // TYPE, MIN, MID, MAX, DEADBAND. See INPUT FORMAT section
    #define PRI_INPUT2            3, -1000, 0, 1000, 100  // TYPE, MIN, MID, MAX, DEADBAND. See INPUT FORMAT section
  #endif
...
*/
  ...
  // #define DUAL_INPUTS                     // ADC*(Primary) + iBUS(Auxiliary). Uncomment this to use Dual-inputs
    ...
    #define CONTROL_ADC           0       // use ADC as input. Number indicates priority for dual-input. Disable CONTROL_SERIAL_USART2, FEEDBACK_SERIAL_USART2, DEBUG_SERIAL_USART2!
    #define CONTROL_SERIAL_USART1 1       // use RC iBUS input on the RIGHT cable. Number indicates priority for dual-input. Disable DEBUG_SERIAL_USART1!
...
// ############################ VARIANT_HOVERCAR SETTINGS ############################
  ...
  #define CONTROL_ADC             0         // use ADC as input. Number indicates priority for dual-input. Disable CONTROL_SERIAL_USART2, FEEDBACK_SERIAL_USART2, DEBUG_SERIAL_USART2!
  #define SIDEBOARD_SERIAL_USART1 1         // Rx from right sensor board: to use photosensors as buttons. Number indicates priority for dual-input. Comment-out if sideboard is not used!
  #define FEEDBACK_SERIAL_USART1            // Tx to   right sensor board: for LED battery indication. Comment-out if sideboard is not used!

  #define DUAL_INPUTS                       // ADC*(Primary) + Sideboard_USART1(Auxiliary). Uncomment this to use Dual-inputs
...
// Balancing controller:  [TODO]
  ...
  #define SIDEBOARD_SERIAL_USART1 0           // right sensor board cable. Number indicates priority for dual-input. Disable if I2C (nunchuk or lcd) is used!
  #define FEEDBACK_SERIAL_USART1
...
// ########################### UART SETIINGS ############################
    ...
    defined(FEEDBACK_SERIAL_USART1) || defined(CONTROL_SERIAL_USART1) || defined(DEBUG_SERIAL_USART1) || defined(SIDEBOARD_SERIAL_USART1)
  #define SERIAL_START_FRAME      0xABCD                  // [-] Start frame definition for serial commands
  #define SERIAL_BUFFER_SIZE      1024                    // [bytes] Size of Serial Rx buffer. Make sure it is always larger than the structure size
...
#endif
#if defined(FEEDBACK_SERIAL_USART1) || defined(CONTROL_SERIAL_USART1) || defined(DEBUG_SERIAL_USART1) || defined(SIDEBOARD_SERIAL_USART1)
  #ifndef USART1_BAUD
    #define USART1_BAUD           115200                  // UART1 baud rate (short wired cable)
  #endif
  #define USART1_WORDLENGTH       UART_WORDLENGTH_8B      // UART_WORDLENGTH_8B or UART_WORDLENGTH_9B
#endif
// ########################### UART SETIINGS ############################
...
// General checks
#if defined(BOARD_SLAVE) && defined(BOARD_MASTER)
  ...
  #error CONTROL_SERIAL_USART1 and SIDEBOARD_SERIAL_USART1 not allowed, choose one.
#endif
...
#if defined(DEBUG_SERIAL_USART1) && defined(FEEDBACK_SERIAL_USART1)
  #error DEBUG_SERIAL_USART1 and FEEDBACK_SERIAL_USART1 not allowed, choose one.
#endif

#if defined(DEBUG_SERIAL_USART2) && defined(DEBUG_SERIAL_USART1)
  #error DEBUG_SERIAL_USART2 and DEBUG_SERIAL_USART1 not allowed, choose one.
#endif
...
// RIGHT cable checks
#if defined(CONTROL_NUNCHUK) && (defined(CONTROL_SERIAL_USART1) || defined(SIDEBOARD_SERIAL_USART1) || defined(FEEDBACK_SERIAL_USART1) || defined(DEBUG_SERIAL_USART1))
  #error CONTROL_NUNCHUK and SERIAL_USART1 not allowed. It is on the same cable.
#endif

#if defined(CONTROL_PPM_RIGHT) && (defined(CONTROL_SERIAL_USART1) || defined(SIDEBOARD_SERIAL_USART1) || defined(FEEDBACK_SERIAL_USART1) || defined(DEBUG_SERIAL_USART1))
  #error CONTROL_PPM_RIGHT and SERIAL_USART1 not allowed. It is on the same cable.
#endif

#if defined(CONTROL_PWM_RIGHT) && (defined(CONTROL_SERIAL_USART1) || defined(SIDEBOARD_SERIAL_USART1) || defined(FEEDBACK_SERIAL_USART1) || defined(DEBUG_SERIAL_USART1))
  #error CONTROL_PWM_RIGHT and SERIAL_USART1 not allowed. It is on the same cable.
#endif
  ...
  #error SUPPORT_BUTTONS_RIGHT and SERIAL_USART1 not allowed. It is on the same cable.
#endif
